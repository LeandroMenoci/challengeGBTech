...
export default function Produtos() {
  ...
  useEffect(() => {
    ...
    fetch('http://www.mocky.io/v2/5b15c4923100004a006f3c07')
      ...
      .then(data => {
        console.log(data.items[0].product.priceSpecification.price),
          setProduct({
            product1: {
              name: data.items[0].product.name,
              image: data.items[0].product.imageObjects[0].small,
              price: data.items[0].product.priceSpecification.price
            },
            product2: {
              name: data.items[1].product.name,
              image: data.items[1].product.imageObjects[0].small,
              price: data.items[1].product.priceSpecification.price
            },
            product3: {
              name: data.items[2].product.name,
              image: data.items[2].product.imageObjects[0].small,
              price: data.items[2].product.priceSpecification.price
            }
          })
      })
  }, [])
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.boxBag}>
        <div>
          <p>{product.product1.name}</p>
          <img src={product.product1.image} alt="" />
          <p>{product.product1.price}</p>
        </div>
        <div>
          <p>{product.product2.name}</p>
          <img src={product.product2.image} alt="" />
          <p>{product.product2.price}</p>
        </div>
        <div>
          <p>{product.product3.name}</p>
          <img src={product.product3.image} alt="" />
          <p>{product.product3.price}</p>
        </div>
      </div>
    </div>
  )
